...
export const getStaticProps = (async (context) => {
  ...
  console.log(data)
  const ranks = data?.ranks?.quick_kor
...
export default function LeaderboardHaerae({
  ...
  function createDetailRanks(ranks: any, flat: boolean) {
    ...
    for (let i = 0; i < ranks?.length ?? 0; i++) {
      ...
      const date = item[1]
      const score = flat ? item[2] : ('quick_kor' in item[2] ? item[2]['quick_kor'] : null)
      if (score == null) {
        continue
      }
      const displayScore = {
        ...
        "RC": NumberFormatter.format(score["haerae_rc"]),
        "KGK": NumberFormatter.format(score["haerae_kgk"]),
        "HI": NumberFormatter.format(score["haerae_hi"]),
        "invis: orig_tag" : tag
      }
  ...
  function createLineRanks(ranks: any, flat: boolean) {
    ...
    for (let i = 0; i < ranks?.length ?? 0; i++) {
      ...
      const display_value = [
        ...
        NumberFormatter.format(score["haerae_overall"]),
        viz_tag
      ]
  ...
  useEffect(() => {
    ...
    const ranks = (session as any)?.ranks?.quick_kor
    if (ranks) {
  ...
  function submitToPublic(event: any): void {
    ...
    fetch(`${process.env.NEXT_PUBLIC_ENDPOINT_URL}/backend/ranking/quick_kor/submit`, {
        method: 'POST',
  ...
  return (
    <Admin>
      ...
      <BYOMModal isBYOMOpen={isBYOMOpen} onBYOMOpen={onBYOMOpen} onBYOMClose={onBYOMClose} quick={true}/>
